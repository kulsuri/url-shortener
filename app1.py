...
def check_table_exists_in_sqlite_db():
    ...
    try:
        ...
        # run_query_db(create_table, 'No Fetch')
...
def create_url(normalized_url):
    ...
    # conn = sqlite3.connect('urls.db')
    # cur = conn.cursor()
    # cur.execute(insert_row, (original_url, shortened_url))
    # conn.commit()
    # conn.close()
    # cur.execute('INSERT INTO shortened_urls (URL, SHORTENED_URL) VALUES ({ou}, {su})'.format(ou=original_url, su=shortened_url))
    # cur.execute("INSERT INTO shortened_urls (URL, SHORTENED_URL) VALUES ({ou}, {su})".format(ou='httpgooglecom', su='twat'))
    ...
    #     cursor = conn.cursor()
    #     insert_row = "INSERT INTO shortened_urls (URL, SHORTENED_URL) VALUES (?, ?)"
    #     cursor.execute(insert_row, ((original_url, shortened_url))
...
def display_url_db():
    ...
    return jsonify(query), 200 #jsonify(url_db) # return the url_db for viewing
# ---------------------------------------------------------------

# GET ROUTE /shortened_url
# ---------------------------------------------------------------
